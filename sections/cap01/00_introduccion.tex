...
\begin{itemize}[noitemsep]
    ...
    \texttt{core} sobre PostgreSQL~15.10 (servidor TIS, phpPgAdmin). Encontrarán el diccionario de datos
...
backend (\texttt{EthosBack}, Spring Boot) y el frontend (\texttt{EthosFront}, React SPA)
en un único artefacto ejecutable, desplegado en el dominio
\url{http://bytebusters.tis.cs.umss.edu.bo}, junto con sus dependencias externas: una
\textbf{base de datos PostgreSQL~15.10} autoalojada en el servidor TIS (administrada vía
phpPgAdmin), \textbf{Supabase} (autenticación y comunicación en tiempo real),
...
\hline
\rowcolor{grisTecnico}
Base de datos & PostgreSQL 15.10 (servidor TIS) & Almacenamiento relacional con RLS y RPCs \texttt{SECURITY DEFINER}; administrada vía phpPgAdmin. \\
\hline
...
\begin{tikzpicture}[
    ...
    \node[fuera] (f2) at (3.2,2.4) {Hosting de la base de datos (servidor TIS — PostgreSQL 15.10)};
